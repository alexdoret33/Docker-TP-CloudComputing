...
$shared_folders = {}
$forwarded_ports = {}
$nomdisk = './deuxiemedisque'
$tailledisk = 10

$script_centos = <<-SCRIPT
 yum install nfs-utils
 mkdir /var/nfs/data
 chmod -R 755 /var/nfs/data && chown nfsnobody:nfsnobody /var/nfs/data
 ...
 echo "/var/nfs/data    172.17.8.*(rw,sync,no_root_squash,no_all_squash)\n/home            172.17.8.*(rw,sync,no_root_squash,no_all_squash)" >> /etc/exports
...
Vagrant.configure("2") do |config|
  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, 10+i] do |config|
      ...
      config.vm.provider :virtualbox do |vb|
        ...
        $nomdisk = "%s%02d%03s" % [$disk, i, '.vdi']
      unless File.exist?($nomdisk)
          vb.customize ['createhd', '--filename', $nomdisk, '--variant', 'Fixed', '--size', $tailledisk * 1024]
          vb.customize ['storagectl', :id, '--name', 'SATA Controller', '--add', 'sata', '--portcount', 4]
          vb.customize ['storageattach', :id,  '--storagectl', 'SATA Controller', '--port', 2, '--device', 0, '--type', 'hdd', '--medium', $nomdisk]
      end
        config.ignition.config_obj = vb
    end

      ip = "172.17.8.#{i+200}"
      config.vm.network :public_network, ip: ip, bridge: "enp3s0"
